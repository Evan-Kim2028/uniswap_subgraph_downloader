...
async def run_query(date_range: tuple[datetime, datetime]) -> pd.DataFrame:
    ...
    async with AsyncSubgrounds() as sg:
        # https://thegraph.com/explorer/subgraphs/A3Np3RQbaBA6oKJgiwDJeo5T3zrYfGHPWFYayMwtNDum?view=Overview&chain=arbitrum-one
        deployment_id: str = "QmXYAmabbEmnSDNR5GWJhmm2NQKXnPkSJEBC3EsMwjXr4e"
        ...
        try:
            ...
            if not os.path.exists(filename):
                ...
                df: pd.DataFrame = await sg.query_df([
                    swaps_qp.transaction._select('id'),
                    ...
                    swaps_qp.sender,
                    swaps_qp.to,
                    swaps_qp.pair._select('id'),
                    swaps_qp.pair.reserve0,
                    swaps_qp.pair.reserve1,
                    swaps_qp.pair.token0.decimals,
                    swaps_qp.pair.token0._select('id'),
                    swaps_qp.pair.token0.name,
                    swaps_qp.pair.token0.symbol,
                    swaps_qp.pair.token1.decimals,
                    swaps_qp.pair.token1._select('id'),
                    swaps_qp.pair.token1.name,
                    swaps_qp.pair.token1.symbol,
                ])
                print(df.shape)
